...
/**
 * ถ้า DB ของคุณไม่มี created_at ให้ใช้ id แทน
 */
$orderBy = "p.id DESC";
  ...
  <aside class="card panel">
    ...
    <form method="get">
      ...
      <div class="row wrap" style="margin-top:14px">
        ...
        <!-- สำคัญ: อย่าใส่ / นำหน้า ถ้าเว็บอยู่ในโฟลเดอร์ /nongkasek/ -->
    ...
    <?php else: ?>
      <div class="grid">
        <?php foreach($products as $p): ?>
          <?php
            // ถ้า functions.php ไม่มี product_cover ให้เพิ่มฟังก์ชันนั้น (ผมให้ไว้ด้านล่าง)
          ...
          <article class="card product">
            <div class="thumb">
              <?php if($cover): ?>
                <img
                  src="<?= h(url('uploads/products/' . $cover)) ?>"
                  alt="<?= h($p['name']) ?>"
                  class="product-img"
                >
              <?php else: ?>
                <div class="small">ไม่มีรูปสินค้า</div>
              <?php endif; ?>
            </div>
            ...
            <div class="row wrap">
              ...
              <form method="post" action="<?= h(url('quick_add.php')) ?>">
                ...
                <button class="btn primary" type="submit">หยิบใส่ตะกร้า</button>
              </form>
            </div>
          </article>
        <?php endforeach; ?>
      ...
      <div class="row between wrap" style="margin-top:14px">
        ...
        <div class="pagination">
          <?php for($i=1;$i<=$totalPages;$i++): ?>
            <a
              class="<?= $i===$page?'active':'' ?>"
              href="<?= h(url('products.php?'.build_qs(['page'=>$i]))) ?>"
            ><?= $i ?></a>
          <?php endfor; ?>
        </div>
      </div>
