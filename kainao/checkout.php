...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($address === '') {
    ...
    try {
      ...
      /**
       * ✅ เปลี่ยนจาก pay.php -> โฟลเดอร์ใหม่ /pay/
       * เดิม: redirect('/pay.php?order_id=' . $orderId);
       * ใหม่: redirect('/pay/?order_id=' . $orderId);
       *
       * หมายเหตุ: /pay/ จะเรียก pay/index.php อัตโนมัติ
       */
      redirect('/pay/?order_id=' . $orderId);
  ...
  <div class="grid2" style="margin-top:14px; gap:18px">
    ...
    <div class="card" style="padding:18px">
      ...
      <div class="small" style="margin-top:8px; opacity:.85">
        หลังยืนยันสั่งซื้อ ระบบจะพาไปหน้า <b>/pay/</b> เพื่อสแกน QR + อัปโหลดสลิป
      </div>
    </div>
  </div>
</div>
