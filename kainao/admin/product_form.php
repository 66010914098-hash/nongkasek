...
function upload_error_text(int $code): string {
  switch ($code) {
    ...
    case UPLOAD_ERR_INI_SIZE: return 'ไฟล์ใหญ่เกิน upload_max_filesize (php.ini)';
    case UPLOAD_ERR_FORM_SIZE: return 'ไฟล์ใหญ่เกิน MAX_FILE_SIZE (ฟอร์ม)';
...
$p = [
  ...
  'name'=>'','slug'=>'','price'=>'0','stock'=>'0','description'=>'','is_active'=>1
];
...
if (isset($_POST['save'])) {
  ...
  if ($name === '') $err = 'กรุณากรอกชื่อสินค้า';
  if ($slug === '') $err = $err ?: 'กรุณากรอก slug';
  ...
  if ($err === '') {
    ...
    if (!empty($_FILES['images']['name'][0])) {
      ...
      // 1) ตรวจ/สร้างโฟลเดอร์
      ensure_dir(PRODUCT_IMG_DIR);

      if (!is_dir(PRODUCT_IMG_DIR)) {
        $err = 'โฟลเดอร์รูปสินค้าไม่มีอยู่: ' . PRODUCT_IMG_DIR;
      } elseif (!is_writable(PRODUCT_IMG_DIR)) {
        $err = 'โฟลเดอร์รูปสินค้าเขียนไม่ได้ (permission): ' . PRODUCT_IMG_DIR;
      } else {
        ...
        for ($i=0; $i<$count; $i++) {
          ...
          if ($file['error'] !== UPLOAD_ERR_OK) {
            $err = 'อัปโหลดรูปไม่สำเร็จ: ' . $file['name'] . ' → ' . upload_error_text($file['error']);
            break;
          }

          // 3) จำกัดขนาด 5MB
          if ($file['size'] > 5 * 1024 * 1024) {
            $err = 'ไฟล์ใหญ่เกิน 5MB: ' . $file['name'];
            break;
          }
          ...
          if (!in_array($mime, $okMime, true)) {
            $err = 'ชนิดไฟล์ไม่รองรับ (ต้องเป็น jpg/png/webp): ' . $file['name'];
            break;
          }

          // 5) เรียก safe_upload + บันทึก DB
          try {
            $webPath = safe_upload($file, ['jpg','jpeg','png','webp'], PRODUCT_IMG_DIR, 'p'.$id);

            if (!$webPath) {
              $err = 'อัปโหลดไม่สำเร็จ (safe_upload คืนค่า empty): ' . $file['name'];
              break;
            }
            ...
            $err = 'อัปโหลดไม่สำเร็จ: ' . $file['name'] . ' | ' . $e->getMessage();
            break;
          }
        }
      }
...
<div class="card panel" style="max-width:980px;margin:0 auto">
  ...
  <?php if($err): ?>
    <div class="toast err" style="margin-top:12px">
      <b>ผิดพลาด</b>
      <div><?= h($err) ?></div>
    </div>
  <?php endif; ?>
